...
```jsonc
{
  ...
  "collection": {
    "g1-hira-あ": { "recovered": true, "facets": { "hira-letter-to-word": true, "hira-word-to-letter": true }, "firstTryCorrect": 1, "correctCount": 2, "missCount": 0, "lastAnsweredAt": "2026-08-23T00:00:00.000Z" }
  },
...
- v2で自動回収された必修項目(`1970-01-01T00:00:00.000Z`の移行印)は、v3以降への移行時に未回収へ戻す。実際の回答日時がある回収記録は残す
- v3のひらがな回収記録は図鑑の表示状態として残すが、観点別記録は空から始める。過去の1問正解を新しいコンプリート条件へ自動変換しない
- 自動回収を解除したステージは未クリアへ戻して再挑戦可能にする。ただし自己ベストの得点・星は残す
- v1からv4へ移行する場合も、クリア済みステージを自動回収せず、必修項目があるステージだけ未クリアへ戻す
...
- ステージ: 島内で直前のステージをクリア(正答率60%以上かつ必修項目100%コンプリート)していれば解放。島をまたぐ依存なし
