...
- v2で自動回収された必修項目(`1970-01-01T00:00:00.000Z`の移行印)は、v3移行時に未回収へ戻す。実際の回答日時がある回収記録は残す
- 自動回収を解除したステージは未クリアへ戻して再挑戦可能にする。ただし自己ベストの得点・星は残す
- v1からv3へ移行する場合も、クリア済みステージを自動回収せず、必修項目があるステージだけ未クリアへ戻す
