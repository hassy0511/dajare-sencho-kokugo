...
- フォント: 子ども向け丸ゴシック(OFL ライセンスの日本語フォント。候補: M PLUS Rounded 1c / Zen Maru Gothic)を**サブセット化して同梱**(ひらがな・カタカナ・教育漢字1026字+記号で数百KBに収める)
...
4. **SVG の扱い**: SVG は `public/assets/svg/` のファイルとして管理し、Phaser の SVG ローダで**デバイス解像度に応じて2倍でラスタライズ**して読み込む。キャラの表情差分は「1ファイル内のグループを showFace(name) で切替」ではなく、**表情ごとに別テクスチャ**(ビルド時にスクリプトで `dj-face-*` グループから自動生成)にして Phaser 側で差し替える
